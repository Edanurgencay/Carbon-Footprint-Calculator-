...
struct LpgMileageInput : View {
    ...
    func calculatePeopleView() -> some View {
        GroupBox {
            ...
            TextField("km", text: $viewModel.lpg)
                .frame(width: 300, height: 50)
                .textFieldStyle(RoundedBorderTextFieldStyle())
                .font(.custom("RobotoSlab-Regular", size: 16))
            
            Button(action: {
                guard let lpg = Int(viewModel.lpg) else {
                    print("Hata: benzin değeri tamsayıya dönüştürülemedi")
                    return
                }
                
                if let uid = currentUserUID {
                    firebaseManager.saveUserDataaLpg(uid: uid, data: ["lpg": lpg]) { error in
                        if let error = error {
...
struct LpgMileageInput_Previews: PreviewProvider {
